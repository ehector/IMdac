...
// [[Rcpp::export]]
arma::vec valid_poisson_individual_IM(const arma::mat & theta, const arma::vec theta_MLE, const arma::vec& Y, const arma::mat covariates,
                                       const double& n, const double& p, const double& M){
  int T = theta.n_rows;
  arma::vec pl = zeros<vec>(T);
  
  arma::mat mu = exp(covariates * theta.t());
  arma::vec mu_MLE = exp(covariates * theta_MLE); 
  
  arma::mat Q, R;
  arma::qr_econ(Q, R, covariates);
  
  for(int t=0; t<T; t++){
    double observed_contour = poisson_individual_IM(mu.col(t), mu_MLE, Y);
    arma::vec res = zeros<vec>(M);
    for(int sim=0; sim<M; sim++){
      arma::vec Ysim = zeros<vec>(n);
      for(int j=0; j<n; j++){
        Ysim(j) = R::rpois(mu(j,t)); 
      }
      ...
      for (int it = 0; it < 10000; it++) {
        ...
        const arma::colvec z = eta + (Ysim - mu) / mu;
        const arma::mat C = arma::chol(Q.t() * (Q.each_col() % mu));
        const arma::colvec s1 = arma::solve(arma::trimatl(C.t()), Q.t() * (mu % z));
        s = arma::solve(arma::trimatu(C), s1);
        eta = Q * s;
      ...
      arma::vec theta_MLE_sim = arma::solve(arma::trimatu(R), Q.t() * eta);
      arma::vec mu_MLE_sim = exp(covariates * theta_MLE_sim);
      double sim_contour = poisson_individual_IM(mu.col(t), mu_MLE_sim, Ysim);
      if(sim_contour <= observed_contour) res(sim) = 1;
    }
    pl(t) = mean(res);
  }
...
arma::vec logistic_pl_mid_exact(arma::mat const& theta, arma::mat const& theta_MLE, int const& k, 
  ...
  arma::field<arma::mat> mu(k), Q(k), R(k);
  for(int i=0; i<k; i++){
    ...
    for(int sim=0; sim<M; sim++){
      ...
      for(int i=0; i<k; i++){
        ...
        if( sum(Ysim_i==0) >= n_i(i)-1 || sum(Ysim_i==1) >= n_i(i)-1 ) {
          ...
          theta_MLE_sim_i = arma::solve(arma::trimatu(R(i)), Q(i).t() * eta);
          theta_MLE_sim.row(i) = theta_MLE_sim_i.t();
          J_ni_sim(i) = zeros<mat>(p,p);
          for(int j=0; j<n_i(i); j++){
...
// [[Rcpp::export]]
arma::vec poisson_pl_mid_exact(arma::mat const& theta, arma::mat const& theta_MLE, int const& k, 
                                arma::field<arma::mat> const& J_ni, arma::vec const& n_i, 
                                arma::field<arma::mat> const& covariates, int const& M){
  int p = theta.n_cols;
  double T = theta.n_rows;
  ...
  arma::field<arma::mat> mu(k), Q(k), R(k);
  for(int i=0; i<k; i++){
    mu(i) = exp(covariates(i) * theta.t()); 
    arma::qr_econ(Q(i), R(i), covariates(i));
  }
  
  for(int l=0; l<T; l++){
    double wl_obs = poisson_work_likeli(theta.row(l).t(), theta_MLE, J_ni, p, k);
    arma::vec res = zeros<vec>(M);
    for(int sim=0; sim<M; sim++){
      arma::mat theta_MLE_sim = zeros<mat>(k,2);
      arma::field<arma::mat> J_ni_sim(k);
      for(int i=0; i<k; i++){
        ...
        arma::colvec eta = arma::ones<arma::colvec>(n_i(i));
        bool is_converged;
        ...
        for(int j=0; j<n_i(i); j++){
          double mu_ij = arma::as_scalar(exp(covariates(i).row(j)*theta_MLE_sim_i));
          J_ni_sim(i) = J_ni_sim(i) + covariates(i).row(j).t()*covariates(i).row(j) * mu_ij;
        } 
      }
      double wl_sim = poisson_work_likeli(theta.row(l).t(), theta_MLE_sim, J_ni_sim, p, k);
      if(wl_sim <= wl_obs) res(sim) = 1;
    }
    pl(l) = mean(res);
  }
...
arma::vec logistic_pl_mid_P(arma::mat const& theta, arma::mat const& theta_dag, arma::mat const& theta_MLE, int const& k, 
  ...
  arma::field<arma::mat> mu_dag(k), Q(k), R(k);
  for(int i=0; i<k; i++){
  ...
  for(int d=0; d<D; d++){
    ...
    for(int sim=0; sim<M; sim++){
      ...
      for(int i=0; i<k; i++){
        ...
        for(int j=0; j<n_i(i); j++){
          Ysim_i(j) = R::rbinom(1, mu_dag(i)(j,d));  
        }
        ...
        arma::colvec s = arma::zeros<arma::colvec>(p);
        arma::colvec s1, s_old, mu, mu_p, z, W;
        arma::colvec eta = covariates(i) * theta_dag.row(d).t();
        bool is_converged, error;
        error = false;
...
// [[Rcpp::export]]
arma::vec poisson_pl_mid_P(arma::mat const& theta, arma::mat const& theta_dag, arma::mat const& theta_MLE, int const& k, 
                            arma::field<arma::mat> const& J_ni, arma::vec const& n_i, 
                            arma::field<arma::mat> const& covariates, int const& M){
  int p = theta.n_cols;
  double T = theta.n_rows;
...
arma::vec poisson_pl_mid_IS(arma::mat const& theta, arma::mat const& theta_dag, arma::mat const& theta_MLE, int const& k, 
  ...
  arma::field<arma::mat> mu_dag(k), Ysim(k), Q(k), R(k);
  for(int i=0; i<k; i++){
    mu_dag(i) = exp(covariates(i) * theta_dag.t()); 
    ...
    for(int d=0; d<D; d++){
      for(int sim=0; sim<M; sim++){
        for(int j=0; j<n_i(i); j++){
          Ysim_i(sim*n_i(i)+j,d) = R::rpois(mu_dag(i)(j,d));  
        }
      }
    }
    Ysim(i) = Ysim_i;
  ...
  for(int l=0; l<T; l++){
    ...
    arma::field<arma::vec> mu(k);
    for(int i=0; i<k; i++){
      mu(i) = exp(covariates(i) * theta.row(l).t()); 
    }
    ...
    for(int sim=0; sim<M; sim++){
      ...
      for(int i=0; i<k; i++){
        arma::vec Ysim_i = Ysim(i)(span(sim*n_i(i), (sim+1)*n_i(i)-1), closest);
        eweights = eweights + logdpois(Ysim_i, mu(i)) - logdpois(Ysim_i, mu_dag(i).col(closest));

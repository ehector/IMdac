...
# Exact middle ground IM
middle_IM_exact <- lognormal_pl_mid_exact(theta=theta_vec, theta_MLE=theta_MLE, k=k, J_ni=J_ni, n_i, M=50000)
...
dat_middle_IM <- data.frame(theta_1=theta_vec[,1], theta_2=theta_vec[,2], type="equilibrated", contour=middle_IM_exact)
...
for(sim in 1:nsim){
  ...
  # optimal_samples[sim] <- valid_lognormal_individual_IM(theta=matrix(theta,nrow=1), Y=unlist(Y.sim), n=sum(n_i), M=50000)
  samples[sim] <- lognormal_pl_mid_exact(theta=matrix(theta,nrow=1), theta_MLE=theta_MLE.sim, k=k, J_ni=J_ni.sim, n_i, M=50000)
